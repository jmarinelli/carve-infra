...
- Offline inspection creation (requires connectivity in MVP).
...
### 8.4 Risk — Photo Upload Performance in the Field

**Risk:** Inspectors work in environments with variable connectivity (garages, outdoor locations). Photo upload during inspection creation may be slow or fail, disrupting the workflow.

**Mitigation:** Implement photo upload with:
- Automatic compression before upload.
- Background upload (photos queue and upload while the inspector continues working).
- Visual confirmation of upload status per photo.
- Graceful handling of failed uploads with retry.
- Support for completing the inspection form while photos are still uploading.
...
**No native mobile app in Phase 1.** The inspection creation flow must work well in a mobile browser. Progressive Web App (PWA) capabilities (offline caching of static assets, add-to-home-screen) are desirable but not required for MVP.
...
### 9.5 OpenGraph and Social Sharing
...
### 9.8 Starter Inspection Template
